...
def combine_cleveland_data(webex_df, user_activity_df):
    ...
    def find_texts(agent_name, normalized_name):
        ...
        Always requires both first and last name to match (with nickname normalization);
        first-name-only fallback intentionally not used because nickname collisions
        (e.g. Steve <-> Stephen) cause false matches across different people.
        """
        # 1. Exact normalized name match
...
def combine_chattanooga_data(webex_df, user_activity_df, tecobi_df):
    ...
    tecobi_avg_durations = [
        find_value(agent, norm, tecobi_avgdur_by_normalized, tecobi_avgdur_by_canonical)
        for agent, norm in zip(webex_df['Agent Name'], webex_df['Name_Normalized'])
    ]
